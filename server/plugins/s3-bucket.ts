...
export default defineNitroPlugin(async () => {
  ...
  if (!env.S3_FORCE_PATH_STYLE) {
    console.log(`[Kush Talents] S3 bucket "${env.S3_BUCKET}" — managed provider detected, skipping initialization`)
  ...
  try {
    ...
    console.log(`[Kush Talents] S3 bucket "${env.S3_BUCKET}" is ready`)
    logInfo('s3.bucket_ready', { bucket: env.S3_BUCKET })
  } catch (error) {
    console.error(`[Kush Talents] Failed to initialize S3 bucket "${env.S3_BUCKET}":`, error)
