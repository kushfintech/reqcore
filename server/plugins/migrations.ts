...
export default defineNitroPlugin(async () => {
  ...
  if (process.env.RAILWAY_ENVIRONMENT_ID) {
    console.log('[Kush ATS] Skipping runtime migrations on Railway (handled in preDeploy)')
  ...
  try {
    ...
    if (!locked) {
      console.log('[Kush ATS] Another instance is running migrations, skipping')
      logInfo('migrations.skipped_locked')
      return
    }

    console.log('[Kush ATS] Running database migrations...')
    ...
    console.log('[Kush ATS] Database migrations applied successfully')
    logInfo('migrations.completed')
  } catch (error) {
    console.error('[Kush ATS] Migration failed:', error)
