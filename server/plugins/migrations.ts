...
export default defineNitroPlugin(async () => {
  ...
  if (process.env.RAILWAY_ENVIRONMENT_ID) {
    console.log('[Kush Talents] Skipping runtime migrations on Railway (handled in preDeploy)')
  ...
  try {
    ...
    if (!locked) {
      console.log('[Kush Talents] Another instance is running migrations, skipping')
      logInfo('migrations.skipped_locked')
      return
    }

    console.log('[Kush Talents] Running database migrations...')
    ...
    console.log('[Kush Talents] Database migrations applied successfully')
    logInfo('migrations.completed')
  } catch (error) {
    console.error('[Kush Talents] Migration failed:', error)
