...
async function sendEmail(msg: EmailMessage): Promise<void> {
  ...
  console.info(`[Kush ATS] ${msg.logFallback}`)
}
...
export async function sendVerificationEmail(data: {
  ...
  try {
    await sendEmail({
      to: data.user.email,
      subject: 'Verify your email address — Kush ATS',
...
export async function sendPasswordResetEmail(data: {
  ...
  try {
    await sendEmail({
      to: data.user.email,
      subject: 'Reset your password — Kush ATS',
...
export async function sendOrgInvitationEmail(data: {
  ...
  await sendEmail({
    to: data.email,
    subject: `You're invited to join ${data.organization.name} on Kush ATS`,
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background-color:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e4e4e7;">
          ...
          <tr>
            <td style="padding:32px 32px 24px;text-align:center;border-bottom:1px solid #f4f4f5;">
              <h1 style="margin:0;font-size:20px;font-weight:600;color:#09090b;">Kush ATS</h1>
            </td>
          </tr>
          <!-- Body -->
          <tr>
          ...
          <tr>
            <td style="padding:16px 32px;text-align:center;border-top:1px solid #f4f4f5;background-color:#fafafa;">
              <p style="margin:0;font-size:12px;color:#a1a1aa;">
                Sent by Kush ATS &mdash; Open-source applicant tracking
              </p>
            </td>
          </tr>
        </table>
    ...
    <tr>
      <td align="center">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background-color:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e4e4e7;">
          ...
          <tr>
            <td style="padding:16px 32px;text-align:center;border-top:1px solid #f4f4f5;background-color:#fafafa;">
              <p style="margin:0;font-size:12px;color:#a1a1aa;">Sent by Kush ATS &mdash; Open-source applicant tracking</p>
            </td>
          </tr>
        </table>
      </td>
...
function buildVerificationText(params: { url: string }): string {
  return [
    ...
    '',
    'Click the link below to verify your email and activate your Kush ATS account:',
    params.url,
    '',
    'If you didn\'t create an account, you can safely ignore this email.',
    '',
...
function buildPasswordResetText(params: { url: string }): string {
  return [
    'Reset your password',
    '',
    'Click the link below to reset your Kush ATS password:',
    params.url,
    '',
      ...
      <td align="center">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background-color:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e4e4e7;">
          ...
          <tr>
            <td style="padding:16px 32px;text-align:center;border-top:1px solid #f4f4f5;background-color:#fafafa;">
              <p style="margin:0;font-size:12px;color:#a1a1aa;">
                Sent by ${escapeHtml(data.organizationName)} via Kush ATS
              </p>
            </td>
          </tr>
        </table>
